...
If `~/.paf/` is absent, continue the project task using this route. When the
...
```text
git clone --branch stable --single-branch ssh://git@project.cureinteractive.com:222/cure-interactive/project-agent-framework.git ~/.paf
```

If it exists, follow `~/.paf/AGENTS.md`, including its non-blocking daily
...
After bootstrap, follow `~/.paf/AGENTS.md` to the applicability route, then
...
discoverable through `~/.paf/skill/INDEX.md` without per-project or
